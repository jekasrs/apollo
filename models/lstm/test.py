"""
Тест сохранённой LSTM (отчёт как у Apollo eval).
...
def main():
    parser = argparse.ArgumentParser(description="LSTM: тест сохранённой модели")
    ...
    if not meta_path.is_file():
        raise SystemExit(
            f"Нет {meta_path}. Сначала: python models/lstm/train.py"
        )
    ...
    model_path = args.model or (ckpt_dir / "best_lstm_meld.keras")
