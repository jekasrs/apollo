...
class SeqContext(nn.Module):
    ...
    def forward(self, text_len_tensor, text_tensor):
        ...
        # pack_padded_sequence требует lengths на CPU (в т.ч. при CUDA/MPS входе).
        lens_cpu = text_len_tensor.detach().cpu()
        packed = pack_padded_sequence(
            text_tensor, lens_cpu, batch_first=True, enforce_sorted=False
        )
