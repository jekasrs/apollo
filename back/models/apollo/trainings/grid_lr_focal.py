"""
Запуск из каталога ``back/`` с ``PYTHONPATH=.`` (или из корня репозитория: ``PYTHONPATH=back``):
...
def _back_root() -> Path:
    """Каталог ``back/`` (пакеты app, dataset, models)."""
    return Path(__file__).resolve().parents[3]


def _repo_root() -> Path:
    """Корень репозитория (родитель ``back/``)."""
...
def main() -> None:
    ...
    back_root = _back_root()
    repo_root = _repo_root()
    train_script = back_root / "models" / "apollo" / "trainings" / "train.py"
    ...
    for lr in GRID_LR:
        for gamma in GRID_FOCAL_GAMMA:
            ...
            subprocess.run(cmd, cwd=str(back_root), env=env, check=True)

            ckpt_path = repo_root / "results" / rid / "model.pt"
