...
Запуск из каталога ``back/`` (``PYTHONPATH=.``) или с ``PYTHONPATH=back`` из корня репозитория:
...
TRAIN = BACK_ROOT / "models" / "apollo" / "trainings" / "train.py"
EVAL = BACK_ROOT / "models" / "apollo" / "trainings" / "eval.py"
OUT_DIR = REPO_ROOT / "results" / "apollo_meld_at_pause_search"


def _run_ckpt_path(idx: int, name: str) -> Path:
    rid = f"apollo_meld_pause_{idx:02d}_{name}"
    return REPO_ROOT / "results" / rid / "model.pt"
...
def main() -> None:
    ...
    env = {**os.environ, "PYTHONPATH": str(BACK_ROOT)}
    ...
    for i, run in enumerate(DEFAULT_RUNS, start=1):
        ...
        subprocess.run(base, cwd=str(BACK_ROOT), env=env, check=True)
        ...
        row = {
            ...
            "checkpoint": str(run_ckpt.relative_to(REPO_ROOT)),
            "test": ev,
        }
        rows.append(row)
        print(
