...
def main():
    ...
    if not meta_path.is_file():
        raise SystemExit(
            f"Нет {meta_path}. Сначала: PYTHONPATH=. python models/cnn/train.py --modalities a"
        )
    ...
    feature_name = meta.get("feature_name", "meld")
    model_path = args.model or (ckpt_dir / f"best_cnn_{feature_name}.keras")
