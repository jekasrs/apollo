...
def _repo_root() -> Path:
    """back/dataset/preprocess/utils/constants.py → корень репозитория (apollo/)."""
...
TEXT_ENCODER_MAX_LENGTH = int(os.environ.get("APOLLO_TEXT_ENCODER_MAX_LENGTH", "256"))

# Fine-tuned на LibriSpeech 960h — публичная модель HF (сильнее голого wav2vec2-base для речи); hidden 768.
WAV2VEC_MODEL_NAME = os.environ.get(
    "APOLLO_WAV2VEC_MODEL",
    "facebook/wav2vec2-base-960h",
)
AUDIO_FEATURE_DIM = 768  # размерность пулинга Wav2Vec2 base family
WAV2VEC_BATCH_SIZE = int(os.environ.get("APOLLO_WAV2VEC_PREPROCESS_BATCH", "8"))
