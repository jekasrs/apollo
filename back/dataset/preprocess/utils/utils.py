...
def text_for_neural_encoder(text: str) -> str:
    """
    Текст для MPNet / SBERT / аналогичных моделей: сохраняем пунктуацию и смысл,
    только Unicode NFKC и схлопывание пробелов (агрессивный ``clean_text`` им вредит).
    """
...
def l2_normalize_rows(a: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    """Построчная L2-нормализация эмбеддингов (удобно перед конкатенацией модальностей)."""
...
def clean_text(text: str, remove_stopwords=True):
    """
    Для классических пайплайнов (BoW / эвристики): нижний регистр, без пунктуации, лемматизация.

    Для трансформерных энкодеров в препроцессе MELD используйте ``text_for_neural_encoder``.
    """
    text = text.lower()
