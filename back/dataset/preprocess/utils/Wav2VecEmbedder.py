...
# Не раздувать padding в батче на GPU/MPS (иначе conv1d падает по памяти).
_PREPROCESS_MAX_AUDIO_SAMPLES = int(SAMPLE_RATE * 30)
...
class Wav2VecEmbedder:
    ...
    def encode_batch(self, waveforms: list[np.ndarray]) -> np.ndarray:
        ...
        for w in waveforms:
            ...
            if x.shape[0] > _PREPROCESS_MAX_AUDIO_SAMPLES:
                x = x[:_PREPROCESS_MAX_AUDIO_SAMPLES]
            trimmed.append(x)

        micro = 2 if self.device.type == "mps" else 8
        parts: list[np.ndarray] = []
        for i in range(0, len(trimmed), micro):
            sub = trimmed[i : i + micro]
