...
# Очень короткие wav → мало фреймов после CNN; SpecAugment падает (mask_length vs sequence_length).
MIN_WAV_SAMPLES = int(SAMPLE_RATE * 0.5)
...
class _AudioDS(Dataset):
    def __init__(self, samples: list, *, max_samples: int | None = None) -> None:
    ...
    def __getitem__(self, i: int) -> tuple:
        ...
        if self._max_samples is not None and y.shape[0] > self._max_samples:
            y = y[: self._max_samples]
        if y.shape[0] < MIN_WAV_SAMPLES:
            y = np.pad(y.astype(np.float32), (0, MIN_WAV_SAMPLES - int(y.shape[0])), mode="constant")
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="auto: cuda>mps>cpu. Wav2Vec на MPS часто упирается в OOM на длинных репликах — используйте cpu или меньший batch.",
    )
    p.add_argument(
        "--max-audio-seconds",
        type=float,
        default=12.0,
        help="Обрезка начала wav при дообучении (меньше padding в батче → ниже пик памяти). 0 = без обрезки.",
    )
    args = p.parse_args()
    ...
    max_samples = (
        int(args.max_audio_seconds * SAMPLE_RATE) if args.max_audio_seconds and args.max_audio_seconds > 0 else None
    )
    log.info(
        "Устройство дообучения Wav2Vec: %s (max_audio_seconds=%s)",
        device,
        args.max_audio_seconds if max_samples else "full",
    )
    ...
    model.to(device)
    # На очень коротких последовательностях time-mask иногда ломает обучение; для CE-дообучения достаточно без них.
    ...
    train_ds = _AudioDS(train, max_samples=max_samples)
    dev_ds = _AudioDS(dev, max_samples=max_samples)
